...
void test_twofish_ctr() {
  Twofish_initialise();

  Twofish_Byte key[] = "ba baa ba baa banana bana naa ba";
  Twofish_Byte plain[] = "city of stars are you shining just for me?";
  Twofish_Byte cipher[42];
  Twofish_Byte iv[] = {0xc9, 0xc5, 0xf7, 0x63, 0x0b, 0x72, 0x90, 0x10, 0x61, 0x23, 0x83, 0x8a, 0xbc, 0xb9, 0x34, 0xcd};

  Twofish_key xkey;

  Twofish_Byte known_cipher[] = {
    0x0d, 0x75, 0x08, 0x4e, 0xa4, 0xd6, 0x0e, 0x5c,
    0xd6, 0x68, 0xde, 0x18, 0xe4, 0xbd, 0x94, 0x58,
    0x44, 0x56, 0x4a, 0x78, 0x20, 0xbb, 0x59, 0xd0,
    0xce, 0x12, 0x43, 0x85, 0xc6, 0xf1, 0xe1, 0xec,
    0x3e, 0x91, 0xfc, 0x44, 0xb4, 0xb5, 0x76, 0x99,
    0xab, 0xff
  };
  Twofish_Byte decrypted[42];

  Twofish_prepare_key(key, 32, &xkey);
  Twofish_encrypt_ctr(&xkey, iv, plain, 42, cipher);

  int rc = mem_isequal(cipher, known_cipher, 42);
  TEST_CHECK(rc == 0);

  Twofish_encrypt_ctr(&xkey, iv, known_cipher, 42, decrypted);

  int rc2 = mem_isequal(plain, decrypted, 42);
  TEST_CHECK(rc2 == 0);
}
...
void test_xsalsa20() {
  ...
  int rc = mem_isequal(c_known, c, 163);
  TEST_CHECK(rc == 0);
}
...
TEST_LIST = {
    ...
    { "Twofish CTR Mode", test_twofish_ctr},
    { 0 }
};
